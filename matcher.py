...
class YoloMatcher(object):
    ...
    def __call__(self, fmp_size, stride, targets):
        ...
        for batch_index in range(bs):
            ...
            # [N,]
            targets_cls = targets_per_image["labels"].numpy()
            # [N, 4]
            targets_box = targets_per_image["boxes"].numpy()

            for gt_box, gt_label in zip(targets_box, targets_cls):
                x1, y1, x2, y2 = gt_box
                # xyxy -> cxcywh
                ...
                if grid_x < fmp_size and grid_y < fmp_h:
                    gt_objectness[batch_index, grid_y, grid_x] = 1.0
                    cls_not_hot = np.zeros(self.num_classes)
                    cls_not_hot[int(gt_label)] = 1.0
                    gt_classes[batch_index, grid_y, grid_x] = cls_not_hot
                    # box
